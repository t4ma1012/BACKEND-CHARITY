...
// Tạo mã displayId tự động VD: CAMP001
...
const createCampaign = async (req, res) => {
  try {
    // Admin không được tạo campaign
    if (req.user.role === 'ADMIN') {
      return res.status(403).json({ message: 'Admin không được phép tạo chiến dịch' });
    }
...
const getCampaignById = async (req, res) => {
  try {
    ...
    res.status(200).json({ campaign, progress });
  } catch (error) {
    res.status(500).json({ message: 'Lỗi server', error: error.message });
  }
};
...
const updateCampaign = async (req, res) => {
  try {
    const campaign = req.campaign; // đã có từ middleware isOwner
    const { title, description } = req.body;

    // Nếu đang ACTIVE → giáng về PENDING, khóa VietQR
    ...
    res.status(200).json({ 
      message: campaign.status === 'PENDING' 
        ? 'Đã cập nhật, chiến dịch đang chờ duyệt lại' 
        : 'Cập nhật thành công',
      campaign 
    });
  } catch (error) {
...
const getDonationsByCampaign = async (req, res) => {
  try {
    ...
    // Ẩn tên donor nếu isAnonymous
    const result = donations.map((d) => {
      const obj = d.toObject();
      if (obj.isAnonymous) {
        obj.donorId = { name: 'Người hảo tâm', avatar: null };
      }
      return obj;
    });
...
const closeCampaign = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Đã đóng chiến dịch', campaign });
  } catch (error) {
    res.status(500).json({ message: 'Lỗi server', error: error.message });
  }
};

module.exports = { createCampaign, getCampaigns, getCampaignById, getDonationsByCampaign, updateCampaign, closeCampaign };